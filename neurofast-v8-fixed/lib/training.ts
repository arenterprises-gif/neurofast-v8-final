...
export async function uploadDatasetToTogether(
  ...
  // ✅ FIX: Use File instead of Blob — properly sets filename in multipart
  // Blob in Vercel Node.js runtime doesn't set filename correctly for Together API
  ...
  const file = new File([jsonlContent], safeFileName, { type: "application/json" });
