...
import { getDatasetSignedUrl } from "./supabase";
...
export async function uploadDatasetToTogether(
  ...
  // Normalize filename — no spaces, force .jsonl extension
  ...
  // Build multipart body manually — FormData in Vercel serverless
  // doesn't set Content-Type boundary correctly for Together API
  const boundary = `----TogetherBoundary${Date.now()}`;

  // ✅ purpose MUST come before file — Together API is strict about field order
  const body =
    `--${boundary}\r\n` +
