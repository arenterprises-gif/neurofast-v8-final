...
// v8 FIX: getQueueStats extracted from Promise.all with 5-second timeout
//         to prevent 300-second serverless timeout on Redis hang
...
async function handler(req: NextRequest): Promise<NextResponse> {
  ...
  // ─── FIX: getQueueStats extracted here with 5-second hard timeout ────────
  // Previously inside Promise.all — if Redis hung (never resolved, never
  // rejected) the entire Promise.all would stall for 300 s → 504 timeout.
  // Now we race it against a 5 s timer so analytics always loads fast.
  ...
  const [
    ...
    db
      .select({
        ...
        callType: aiCostLogs.callType,
        costUsd:  sum(aiCostLogs.costUsd),
        tokens:   sum(aiCostLogs.totalTokens),
        calls:    count(),
      })
      .from(aiCostLogs)
      .where(gte(aiCostLogs.createdAt, since))
      .groupBy(aiCostLogs.model, aiCostLogs.callType)
    ...
    db
      ...
      .where(
        and(
          eq(trainingJobs.status, "failed"),
          gte(trainingJobs.createdAt, since)
        )
      ),
    db
      .select({ n: count() })
    ...
    db
      ...
      .where(eq(fineTunedModels.status, "active")),
    ...
    // Top job users
    db
      .select({ userId: trainingJobs.userId, jobCount: count() })
      .from(trainingJobs)
      .where(gte(trainingJobs.createdAt, since))
      .groupBy(trainingJobs.userId)
      .orderBy(desc(count()))
      .limit(5),
  ...
  // Enrich top job users with user details
  const topUsersEnriched = await Promise.all(
    topJobUsersRows.map(async (row) => {
      const [u] = await db
        .select({ email: users.email, name: users.name, plan: users.plan, creditsBalance: users.creditsBalance })
        .from(users)
  ...
  const successRate      =
    totalResolved > 0 ? Math.round((completedN / totalResolved) * 100) : null;
  ...
  return NextResponse.json({
    ...
    users: {
      total:       totalUsersRows[0]?.n  ?? 0,
      newInPeriod: newUsersRows[0]?.n    ?? 0,
      daily:       dailyUsersRows,
    },

    revenue: {
      allTimeInr:    parseFloat(String(totalRevenueRows[0]?.total  ?? "0")),
      inPeriodInr:   parseFloat(String(recentRevenueRows[0]?.total ?? "0")),
      daily:         dailyRevenueRows.map((r) => ({
        date:         r.date,
        inr:          parseFloat(String(r.revenue ?? "0")),
    ...
    training: {
      totalAllTime:     totalJobRows[0]?.n       ?? 0,
      completedInPeriod: completedN,
      failedInPeriod:    failedN,
      currentlyRunning:  runningJobRows[0]?.n    ?? 0,
      successRate,
      activeModels:      totalModelsRows[0]?.n   ?? 0,
    },

    credits: {
      purchasedInPeriod: creditsPurchased,
      spentInPeriod:     creditsSpent,
      netFlowInPeriod:   creditsPurchased - creditsSpent,
    },

    errors: {
      countInPeriod: errorCountRows[0]?.n ?? 0,
    },

    topUsers: topUsersEnriched,

    recentActivity: recentAuditRows.map((l) => ({
      id:          l.id,
      action:      l.action,
      severity:    l.severity,
      actorEmail:  l.actorEmail,
      resource:    l.resource,
      // FIX: createdAt is Date from Drizzle — convert safely
